...
public static partial class S
{
    ...
    { 
        ...
        { Home, new("Heim") },
        { HomeBody, new("Dies ist eine Projektmanagement-App, mit der Sie Aufgaben in einer Baumstruktur wie einem Dateiverzeichnis organisieren. Jede Aufgabe enthält eine Zusammenfassung der wichtigen Statistiken der darunter liegenden Aufgaben, der geschätzten und angefallenen Zeit und Kosten usw.") },
        ...
        { OrgConfirmDeactivateMember, new("<p>Sind Sie sicher, dass Sie das Mitglied <strong>{{Name}}</strong> deaktivieren möchten?</p>") },
        { OrgConfirmDeleteOrg, new("<p>Sind Sie sicher, dass Sie die Organisation <strong>{{Name}}</strong> löschen möchten?</p><p>Dies kann nicht rückgängig gemacht werden.</p>") },
        { OrgConfirmDeleteProject, new("<p>Sind Sie sicher, dass Sie das Projekt <strong>{{Name}}</strong> löschen möchten?</p><p>Dies kann nicht rückgängig gemacht werden.</p>") },
        { OrgMemberInviteEmailHtml, new("<p>Lieber <strong>{{InviteeName}}</strong></p><p><strong>{{InvitedByName}}</strong> hat Sie eingeladen, der Organisation beizutreten: <strong>{{OrgName} </strong></p><p><a href=\"{{BaseHref}}/verify_email?email={{Email}}&code={{Code}}\">Bitte klicken Sie auf diesen Link, um Ihre zu bestätigen E-Mail-Adresse und treten Sie <strong>{{OrgName}}</strong></a></p> bei") },
        { OrgMemberInviteEmailSubject, new("{{OrgName}} – Projektmanagement-Einladung") },
        { OrgMemberInviteEmailText, new("Lieber {{InviteeName}}\n\n{{InvitedByName}} hat Sie eingeladen, der Organisation beizutreten: {{OrgName}}\n\nBitte klicken Sie auf diesen Link, um Ihre E-Mail-Adresse zu bestätigen und {{OrgName}} beizutreten:\n\n{{BaseHref}}/verify_email?email={{Email}}&code={{Code}}") },
        ...
        { TaskConfirmDeleteFile, new("<p>Sind Sie sicher, dass Sie die Datei <strong>{{Name}}</strong> löschen möchten?</p>") },
        { TaskConfirmDeleteTask, new("<p>Sind Sie sicher, dass Sie die Aufgabe <strong>{{Name}}</strong> löschen möchten?</p><p>Dies kann nicht rückgängig gemacht werden.</p>") },
        { TaskConfirmDeleteVitem, new("<p>Sind Sie sicher, dass Sie <strong>{{Value}}</strong>?</p>") },
        { TaskDeleteProjectAttempt, new("Der Projektknoten kann nicht vom Aufgabenendpunkt gelöscht werden. Verwenden Sie den Projektlöschendpunkt") },
        { TaskEditTask, new("Update-Aufgabe") },
        { TaskMovePrevSibParentMismatch, new("Frühere Geschwister- und Eltern-IDs stimmen nicht überein") },
        ...
        { TaskRecursiveLoopDetected, new("Die Move-Operation würde zu einer rekursiven Schleife führen") },
        { TaskTooManyDescn, new("Zu viele Nachkommen, um alle zu erhalten, nur gültig für Aufgaben mit 1000 oder weniger Nachkommen") },
        { TaskTooManyDescnToDelete, new("Eine Aufgabe mit {{Max}} oder mehr untergeordneten Aufgaben kann nicht gelöscht werden") },
