...
public static partial class S
{
    ...
    { 
        ...
        { Home, new("Hogar") },
        { HomeBody, new("Oak es una aplicación de gestión de proyectos, las tareas se organizan en árboles como un directorio de archivos. Cada tarea contiene un resumen de las estadísticas de todas las tareas subyacentes, como el tiempo estimado y el tiempo incurrido.") },
        ...
        { OrgConfirmDeactivateMember, new("<p>¿Estás seguro de que deseas desactivar el miembro <strong>{{Name}}</strong>?</p>") },
        { OrgConfirmDeleteOrg, new("<p>¿Está seguro de que desea eliminar la organización <strong>{{Name}}</strong>?</p><p>Esto no se puede deshacer.</p>") },
        { OrgConfirmDeleteProject, new("<p>¿Está seguro de que desea eliminar el proyecto <strong>{{Name}}</strong>?</p><p>Esto no se puede deshacer.</p>") },
        { OrgMemberInviteEmailHtml, new("<p>Estimado <strong>{{InviteeName}}</strong></p><p><strong>{{InvitedByName}}</strong> te ha invitado a unirte a la organización: <strong>{{OrgName} }</strong></p><p><a href=\"{{BaseHref}}/verify_email?email={{Email}}&code={{Code}}\">Haga clic en este enlace para verificar su dirección de correo electrónico y únete a <strong>{{OrgName}}</strong></a></p>") },
        { OrgMemberInviteEmailSubject, new("{{OrgName}} - Invitación a la gestión de proyectos") },
        { OrgMemberInviteEmailText, new("Estimado, {{InviteeName}}\n\n{{InvitedByName}} lo ha invitado a unirse a la organización: {{OrgName}}\n\nHaga clic en este enlace para verificar su dirección de correo electrónico y unirse a {{OrgName}}:\n\n{{BaseHref}}/verify_email?email={{Email}}&code={{Code}}") },
        ...
        { ProjectFileLimitExceeded, new("Se excedió el límite de archivos del proyecto {{FileLimit}}") },
        ...
        { StringValidation, new("Cadena no válida {{Nombre}}, Min {{Min}}, Max {{Max}}, Regexes {{Regexes}}") },
        { SubCounts, new("Conteos secundarios") },
        { Task, new("Tarea") },
        { TaskCantMoveRootProjectNode, new("No se puede mover el nodo del proyecto raíz") },
        { TaskConfirmDeleteComment, new("<p> ¿Estás seguro de que quieres eliminar este comentario? </p>") },
        { TaskConfirmDeleteFile, new("<p>¿Está seguro de que desea eliminar el archivo <strong>{{Name}}</strong>?</p>") },
        { TaskConfirmDeleteTask, new("<p>¿Está seguro de que desea eliminar la tarea <strong>{{Name}}</strong>?</p><p>Esto no se puede deshacer.</p>") },
        { TaskConfirmDeleteVitem, new("<p>¿Está seguro de que desea eliminar <strong>{{Value}}</strong>?</p>") },
        { TaskDeleteProjectAttempt, new("No se puede eliminar el nodo del proyecto del punto final de tareas, use el punto final de eliminación del proyecto") },
        { TaskEditTask, new("Actualizar tarea") },
        { TaskMovePrevSibParentMismatch, new("Las identificaciones de padres y hermanos anteriores no coinciden") },
        ...
        { TaskRecursiveLoopDetected, new("La operación de movimiento daría como resultado un bucle recursivo") },
        { TaskTooManyDescn, new("Demasiados descendientes para obtenerlos todos, solo válido en tareas con 1000 o menos descendientes") },
        { TaskTooManyDescnToDelete, new("No se puede eliminar una tarea con {{Max}} o más tareas descendientes") },
